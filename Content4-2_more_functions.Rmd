...
We can combine multiple logical expressions using the operator `||` for *or* and the operator `&&` for *and*.  Notice that these are different that the symbols we used for *or* and *and* in the `dplyr` functions.  In `if` statements, you want to use the double `||` and `&&`.  The reason for this is nicely explained in "R for Data Science:"
