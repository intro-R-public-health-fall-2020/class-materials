...
*You can see there are no individuals who had never smoked and spend more than $10,000.*
...
*You should see here that the 5 individuals with total expenditures of 5 are not arranged in order of increasing BMI value, with the `NA` value at the end.*
...
And, again, remember that if you want to save your modified dataset so you can work with it, you need to assign the new dataset to an object with the assignment operator, `<-`.  Here we assign it back to the same names of `nmes.data` after arranging the columns in a way we like better.
```{r}
...
2. What does the `any_of()` function do? Run the code below to figure it out:
```{r}
