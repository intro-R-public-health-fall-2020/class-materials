...
This happened because the data was sorted to put the never smokers first, and so `current` was `NA` for all of these first 2084 individuals.  Since `readr` guesses based on the first 1000 rows, it guessed the wrong data type.  We can fix it as we did above using `current = col_character()`:
```{r}
nmesData <- read_csv("data/nmesPROCproblems.csv",
                     col_types = cols(
                       id = col_double(),
