...
Again, we should always be careful when we combine categories together.  Using `fct_lump()` is a tool for making tables and plot simpler by having a smaller number of categories, but the resulting "Other" category has lost information about the individual categories that have been lumped together!
...
Eventually, you may become confidence enough in your coding to simply recode the original variable right away, without checking the new values with the old ones, s below.  However, once you're starting it can be useful to check your recoding as you go, so you are sure you are re-coding the way you think you are re-coding!
```{r}
nmesRAW <- read_csv("data/nmesUNPROC.csv")
...
(2) (2) Create a new factor variable named `beltAlways` that has two levels: `Always` which includes the "Always/Almost always" levels and `Not Always` which has the other 2 levels.
```{r}
