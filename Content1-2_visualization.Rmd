...
The `coord_cartesian()` option is the default option, so doesn't generally need to be specified in your graph.  Howevr, it can also be used to **zoom in** on a specific region of the graph.  Here we want to zoom in on our larger plot to see the section for ages 40 through 60.
```{r}
ggplot(data = nmes.data) +
  geom_point(mapping = aes(x = age, y = bmi)) +
...
Suppose we want to look at the relationship between a continuous variable (`totalexp`) and a categorical variable (`eversmk`).  We can see from the cheatsheet at [https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) that one of the geoms to show this type of relationship is `geom_boxplot()`.  We can start with the basic ggplot structure:
```{r}
ggplot(data = nmes.data) +
  geom_boxplot(mapping = aes(x = eversmk, y = totalexp))
```

Maybe then we want to flip out coordinate system so the boxplots go the other direction:
```{r}
ggplot(data = nmes.data) +
  geom_boxplot(mapping = aes(x = eversmk, y = totalexp)) + 
  coord_flip()
